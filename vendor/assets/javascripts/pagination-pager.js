...
var PaginationPageController = Ember.ObjectController.extend({
  ...
  actions: {
    setCurrent: function () {
      // This part is not original
      route = Dashboard.__container__.lookup('router:main');
      route.send('gotoPage', this.get('content'))
  ...
  actions: {
    next: function () {
      if (!this.get('isLast')) {
        ...
        // This part is not original
        route = Dashboard.__container__.lookup('router:main');
        route.send('gotoPage', parseInt(current, 10) + 1);
      }
    },
